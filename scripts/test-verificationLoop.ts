...
async function main(): Promise<void> {
  ...
  const wrong = evaluateInteraction(
    obs("e17", "Attach files"),
    stepTargeting("e3"),
  );
  ...
  assert(
    hitTest(tree, 630, 900)?.id === "e17",
    "click inside a button hits it",
  );
  ...
  assert(
    d2.action === "correct",
    "interacting with a different element corrects",
  );
  assert(
    getCurrentStep()!.status === "corrected",
    "current step marked corrected",
  );
  assert(
    typeof getCurrentStep()!.correction === "string",
    "correction message set",
  );
  assert(
    events.some((e) => e.type === "step_corrected"),
    "emits step_corrected",
  );
